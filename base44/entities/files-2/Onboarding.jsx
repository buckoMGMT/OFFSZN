...
// OFFSZN onboarding — runs immediately after account creation.
//
// FLOW (one question per screen — Hick's law):
//   0 Welcome        — endowed progress: "Account created ✓" already filled
//   1 Date of birth  — THE AGE GATE. Calls the ageGate backend function.
//                      Under 13 = hard stop. 13-17 = continues, flagged minor.
//   2 Name           — display_name (the only schema-required field)
//   3 Sport          — sport / position / grade
//   4 Goals          — training_goals (multi-select; drives macro targets)
//   5 Body           — weight, height, goal weight
//   6 Weight room    — bench / squat / deadlift / mile (fully skippable)
//   7 Coach or athlete — sets role; coach sees "verification required" truthfully
//   8 Finish         — computes macro targets, writes the Athlete record,
//                      onboarding_complete = true, celebration, into the app.
//
// EVERY field written below exists in Athlete.jsonc TODAY, except
// training_goals — add that one field (snippet in the README).
//
// Import conventions below are Base44's standard SDK paths. If your
// project exposes them differently, only the two import lines change.

import { useState, useMemo } from "react";
import { useNavigate } from "react-router-dom";
import { motion, AnimatePresence, useReducedMotion } from "framer-motion";
import { Athlete, User } from "@/api/entities";
import { ageGate } from "@/api/functions";
...
/* Macro targets from weight + primary goal — standard athlete formulas.
   Editable later in Track settings; this just gives Day 1 real numbers
   instead of zeros (a real sports app never starts you at 0/0/0/0). */
function computeMacroTargets(weightLbs, goals) {
  ...
  const protein = Math.round(weightLbs * 1.0);           // 1g / lb — athlete standard
...
function ProgressBar({ step }) {
  ...
  // so the bar is never empty — people finish journeys already started.
  const filled = step + 1;
  return (
...
function StepShell({ eyebrow, title, children, onBack, canBack }) {
  return (
    <div className="flex flex-col min-h-full px-5 pt-6 pb-8" style={{ maxWidth: 480, margin: "0 auto" }}>
      {canBack && (
      ...
      <span className="eyebrow text-accent text-xs font-semibold uppercase" style={{ letterSpacing: "0.14em" }}>
        {eyebrow}
      </span>
...
export default function Onboarding() {
  ...
  async function submitDob() {
    ...
    try {
      const { data } = await ageGate({ date_of_birth: form.dob });
  ...
  /* -- Step 8: write the Athlete record — every field exists in schema -- */
  async function finish() {
    ...
    try {
      ...
      const payload = {
        ...
        training_goals: form.goals,                       // ← the ONE new schema field
        ...
        // identity_status === "verified" + Stripe onboarding — the role
        // alone unlocks nothing money-related. That's by design.
      ...
      // Update the existing Athlete record if one exists, else create.
      const me = await User.me();
      const existing = await Athlete.filter({ created_by: me.email });
      if (existing?.length) {
        await Athlete.update(existing[0].id, payload);
      } else {
