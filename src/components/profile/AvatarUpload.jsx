...
// Tappable avatar — opens the device photo library / camera, lets the
// athlete CROP the shot square, uploads, and saves to the athlete card.
export default function AvatarUpload({ athlete, onUpdated, editable = false }) {
  ...
  return (
    <>
      ...
      {cropFile && (
        <ImageCropModal file={cropFile} onCropped={handleCropped} onClose={() => setCropFile(null)} />
      )}
    </>
  );
}