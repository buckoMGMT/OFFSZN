...
export default function PlayDiagram({ size = 180, className = "" }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 180 180" fill="none" className={className}>
      ...
      {[50, 90, 130].map((cx, i) => (
        <g key={i}>
          <line x1={cx-6} y1={134} x2={cx+6} y2={146} stroke="#D7263D" strokeWidth="2" />
          <line x1={cx+6} y1={134} x2={cx-6} y2={146} stroke="#D7263D" strokeWidth="2" />
        </g>
      ))}
      ...
      {drawn && <>
        {/* Curl route left */}
        <path d="M38 102 L38 68 Q38 52 54 52 Q68 52 68 66" stroke="#D7263D" strokeWidth="1.5" fill="none"
          strokeDasharray="200" strokeLinecap="round"
          style={{ animation: 'drawRoute 0.9s ease-out 0.1s both' }} className="draw-route" />
        <polygon points="68,60 64,72 72,70" fill="#D7263D" style={{ opacity: drawn ? 1 : 0, transition: 'opacity 0.3s 0.9s' }} />

        {/* Fly route right */}
        <path d="M142 102 L142 46" stroke="#D7263D" strokeWidth="1.5" fill="none"
          strokeDasharray="200" strokeLinecap="round"
          style={{ animation: 'drawRoute 0.7s ease-out 0.3s both' }} className="draw-route" />
        <polygon points="142,40 138,52 146,52" fill="#D7263D" style={{ opacity: drawn ? 1 : 0, transition: 'opacity 0.3s 0.8s' }} />

        {/* Crossing route */}
        <path d="M62 102 L62 74 L118 74" stroke="#D7263D" strokeWidth="1.5" fill="none"
          strokeDasharray="200" strokeLinecap="round"
          style={{ animation: 'drawRoute 1s ease-out 0.5s both' }} className="draw-route" />
        <polygon points="124,74 112,70 112,78" fill="#D7263D" style={{ opacity: drawn ? 1 : 0, transition: 'opacity 0.3s 1.3s' }} />
      </>}
    </svg>
  );
}